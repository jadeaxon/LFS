#!/usr/bin/env bash

#==============================================================================
# Variables
#==============================================================================

set -e

src='../../../sources'
src=$(cd $src; pwd) # Convert to absolute path.

S=$(basename $0)

bpkg=gawk # Pkg base name.
ver=4.1.1 # Pkg version.
pkg=${bpkg}-${ver} # Full pkg name.
z=xz # Compression style.
tarball=${pkg}.tar.$z # Tarball filename.


#==============================================================================
# Main
#==============================================================================

# Extract source tarball.
rm -rf $pkg/
tar xavf $src/$tarball
cd $pkg/

# Apply the batch.
if [ -f $src/${bpkg}*.patch ]; then
	echo "$S: Patching gawk."
	sleep 1
	cp $src/${bpkg}*.patch .
	patch -p1 < ${bpkg}*.patch
fi

./configure --prefix=/tools
make
make check || true
# Note that 5 tests fail.  These seem to have to do with i&ln.
# Also the 'time' test sometimes fails.
# Can't figure out how to fix--assuming gawk works well enough to proceed.
make install

cd ..
rm -rf $pkg/
echo "$S: You have built $pkg from scratch!"


